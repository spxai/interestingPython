...
def drawMainBoard(screen):
    screen.blit(bjpic,(0,0)) 
    gameRectWidth=gameRectHeight=mainWidth-INTERSPACE*2
    pygame.draw.rect(screen,'red2',(INTERSPACE,INTERSPACE,gameRectWidth,gameRectHeight),width=2)
    cellWidth=cellHeight=gameRectWidth/CELLCOUNT
    pygame.display.update()
    return (cellWidth,cellHeight) 

def initCells(screen):
    cellW,cellH=drawMainBoard(screen)
    cellInfo=[[[random.randint(1,8),(cellW*j+INTERSPACE,cellH*i+INTERSPACE)] for i in range(CELLCOUNT)] for j in range(CELLCOUNT)]   
    for i in range(CELLCOUNT):
        for j in range(CELLCOUNT):
            drawCell(screen,i,j,pics[cellInfo[i][j][0],cellW,cellH]
    return cellInfo,cellW,cellH

def drawCell(screen,i,j,pic,cellWidth,cellHeight):
    screen.blit(pic,cellsInfo[i][j][1])
    pygame.display.flip()
...
logo = pygame.image.load(sys.path[0]+r"\pic\3-2-1\game1.png")
pygame.display.set_icon(logo)
bjpic = pygame.image.load(sys.path[0]+r"\pic\3-2-1\bj.jpg")
